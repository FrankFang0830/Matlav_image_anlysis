...
%imposter
%we obtain imposter matrix by computing the distance from one subject to
%the remianing 39 subjects.
ldaimposter=[]
pcaimposter=[]
 for j=1:cat
   for i=1:cat
 ...
 %merge the imposter and genuine into one scores matrix
...
%plot the roc with ezroc3 separately
%obtain arguments which are useful for the following question
...
%plot the comperhensive roc curve into one image.
...
%EXPERIMENT2
%locate the index at a specific FAR value and get the accuracy
...
%find the threshold to classify the samples
pcathr=pcaALLthr(pcalo1);
ldathr=ldaALLthr(ldalo1);
%if the distance>threshold => 0
%else 1
...
%decision rule if lda=pca=0,final=0 ;else if lda=pca=1,final=1
%else randomly select one value from either pca or lda
...
%calculate true postive/negative ;false postive/negative to get final
%accuracy
TP=0
FN=0
for i=1:5
